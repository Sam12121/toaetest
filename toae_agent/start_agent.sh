...
usage() {
  cat <<EOF
    ...
    Please refer toae command line reference guide for more details.
...
launch_toaed() {
  ...
  envsubst '${TOAE_KEY}:${MGMT_CONSOLE_URL}:${MGMT_CONSOLE_PORT}:${MGMT_CONSOLE_URL_SCHEMA}:${SCOPE_HOSTNAME}' </etc/td-agent-bit/fluentbit-agent.conf >/etc/td-agent-bit/fluentbit-agent-new.conf
  mv /etc/td-agent-bit/fluentbit-agent-new.conf /etc/td-agent-bit/fluentbit-agent.conf

  envsubst '${TOAE_KEY}:${MGMT_CONSOLE_URL}:${MGMT_CONSOLE_PORT}:${MGMT_CONSOLE_URL_SCHEMA}:${SCOPE_HOSTNAME}' </etc/td-agent-bit/fluentbit-cluster-agent.conf >/etc/td-agent-bit/fluentbit-cluster-agent-new.conf
...
create_cgroups() {
  ...
  /bin/sh /home/toae/create-cgroups.sh >/dev/null 2>&1
}
