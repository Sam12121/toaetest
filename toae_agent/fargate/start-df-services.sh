...
setup_env_paths() {
  ...
  echo "Toae agent install dir: $DF_INSTALL_DIR"

  export PATH=$DF_INSTALL_DIR/bin:$DF_INSTALL_DIR/usr/local/bin:$DF_INSTALL_DIR/home/toae:$PATH
  echo $PATH
  ...
  export DF_TLS_ON="1"
  export SECRET_SCANNER_LD_LIBRARY_PATH=$DF_INSTALL_DIR/home/toae/lib:$LD_LIBRARY_PATH
}
...
launch_discovery() {
  echo "Starting toae-agent discovery... with hostname $SCOPE_HOSTNAME"
  bash -x $DF_INSTALL_DIR/home/toae/run_discovery_loop.sh &
}

launch_package_scanner() {
  echo "Launching package-scanner grpc server"
  bash -x -c "rm -rf /tmp/package-scanner.sock && $DF_INSTALL_DIR/home/toae/package-scanner -socket-path /tmp/package-scanner.sock -mode grpc-server" &
}
...
chmod +x $DF_INSTALL_DIR/home/toae/*.sh
