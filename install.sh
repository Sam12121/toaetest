...
while [[ $# -gt 0 ]]; do
    ...
    case $key in
        ...
        echo "  -t, --image-tag      Specify the image tag for the Toae components (default: $DEFAULT_IMAGE_TAG) (optional)"
...
kubernetes_deployment() {
    ...
    CONSOLE_VALUES_FILE="deployment-scripts/helm-charts/toae-console/values.yaml"
    ROUTER_VALUES_FILE="deployment-scripts/helm-charts/toae-router/values.yaml"
    ...
    # Install toae-router
    helm upgrade --install toae-router deployment-scripts/helm-charts/toae-router --set global.imageTag="$IMAGE_TAG" --set global.storageClass="$STORAGE_CLASS" -f $ROUTER_VALUES_FILE

    # Install toae-console
    helm upgrade --install toae-console deployment-scripts/helm-charts/toae-console --set global.imageTag="$IMAGE_TAG" --set global.storageClass="$STORAGE_CLASS" --set neo4j.secrets.NEO4J_AUTH=neo4j/$NEO4J_PASSWORD -f $CONSOLE_VALUES_FILE
