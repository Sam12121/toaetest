...
func initMinio() MinioConfig {
	...
	if minioUser == "" {
		minioUser = "toae"
		log.Warn().Msgf("TOAE_MINIO_USER defaults to: %v", minioUser)
	}
...
func initPosgresql() PostgresqlConfig {
	...
	if postgresUsername == "" {
		postgresUsername = "toae"
		log.Warn().Msgf("TOAE_POSTGRES_USER_DB_USER defaults to: %v", postgresUsername)
	}
	postgresPassword := os.Getenv("TOAE_POSTGRES_USER_DB_PASSWORD")
	if postgresPassword == "" {
		postgresPassword = "toae"
...
func initNeo4j() Neo4jConfig {
	...
	if neo4jUsername == "" {
		neo4jUsername = "neo4j"
		log.Warn().Msgf("TOAE_NEO4J_USER defaults to: %v", neo4jUsername)
	}
