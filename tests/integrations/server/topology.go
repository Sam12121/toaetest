...
import (
	...
	"github.com/Sam12121/toaetest/tests_integrations/utils"
)

func Test_topology(setup utils.GraphDBSetup) (time.Duration, error) {
	res, duration, err := utils.RunToaeCtl([]string{"graph", "topology"})
	if err != nil {
		log.Fatal(err)
	}
...
func Test_topology_hosts(setup utils.GraphDBSetup) (time.Duration, error) {
	res, duration, err := utils.RunToaeCtl([]string{"graph", "topology", "--root=hosts"})
	if err != nil {
		log.Fatal(err)
	}
	nodes, has := res["nodes"].(map[string]interface{})
...
func Test_topology_containers(setup utils.GraphDBSetup) (time.Duration, error) {
	res, duration, err := utils.RunToaeCtl([]string{"graph", "topology", "--root=containers"})
	if err != nil {
		log.Fatal(err)
	}
	nodes, has := res["nodes"].(map[string]interface{})
...
func Test_topology_pods(setup utils.GraphDBSetup) (time.Duration, error) {
	res, duration, err := utils.RunToaeCtl([]string{"graph", "topology", "--root=pods"})
	if err != nil {
		log.Fatal(err)
	}
	nodes, has := res["nodes"].(map[string]interface{})
...
func Test_topology_kubernetes(setup utils.GraphDBSetup) (time.Duration, error) {
	res, duration, err := utils.RunToaeCtl([]string{"graph", "topology", "--root=kubernetes"})
	if err != nil {
		log.Fatal(err)
	}
	nodes, has := res["nodes"].(map[string]interface{})
