...
func Test_search_top_vuln_hosts(setup utils.GraphDBSetup) (time.Duration, error) {
	res, duration, err := utils.RunToaeCtl([]string{"top", "--toptype=vulnerability", "--type=host", "--num=5"})
	if err != nil {
		return duration, err
	}
...
func Test_search_all_vulnerabilities(setup utils.GraphDBSetup) (time.Duration, error) {
	res, duration, err := utils.RunToaeCtl([]string{"issues", "--type=vulnerability"})
	if err != nil {
		return duration, err
	}
...
func Test_search_all_vulnerability_count(setup utils.GraphDBSetup) (time.Duration, error) {
	res, duration, err := utils.RunToaeCtl([]string{"count", "--type=vulnerability"})
	if err != nil {
		return duration, err
	}
