...
import (
	...
	"github.com/Sam12121/toaetest/toae_ctl/output"
	ctl "github.com/Sam12121/toaetest/toae_utils/controls"
	"github.com/Sam12121/toaetest/toae_utils/log"
	toae_server_client "github.com/Sam12121/golang_toae_sdk/client"
)
...
var scanStartSubCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		identifiers := []toae_server_client.ModelNodeIdentifier{}
		for i := range scan_node_ids {
			identifiers = append(identifiers, toae_server_client.ModelNodeIdentifier{
		...
		var err error
		var res *toae_server_client.ModelScanTriggerResp
		switch scan_type {
		case "secret":
			req := http.Client().SecretScanAPI.StartSecretScan(context.Background())
			req = req.ModelSecretScanTriggerReq(
				*toae_server_client.NewModelSecretScanTriggerReq(
					*toae_server_client.NewModelScanFilterWithDefaults(),
					identifiers,
				))
		...
		case "malware":
			...
			req = req.ModelMalwareScanTriggerReq(
				*toae_server_client.NewModelMalwareScanTriggerReq(
					*toae_server_client.NewModelScanFilterWithDefaults(),
					identifiers,
				))
		...
		case "vulnerability":
			...
			req = req.ModelVulnerabilityScanTriggerReq(
				*toae_server_client.NewModelVulnerabilityScanTriggerReq(
					*toae_server_client.NewModelScanFilterWithDefaults(),
					identifiers,
					[]toae_server_client.ModelVulnerabilityScanConfigLanguage{*toae_server_client.NewModelVulnerabilityScanConfigLanguage(vuln_scan_type)},
				))
		...
		case "compliance":
			...
			req = req.ModelComplianceScanTriggerReq(
				*toae_server_client.NewModelComplianceScanTriggerReq(
					strings.Split(scan_config, ","),
					*toae_server_client.NewModelScanFilterWithDefaults(),
					[]toae_server_client.ModelNodeIdentifier{
						{
							NodeId:   scan_node_id,
							NodeType: resource_type,
						},
...
var scanStatusSubCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		var err error
		var res *toae_server_client.ModelScanStatusResp
		var res2 *toae_server_client.ModelComplianceScanStatusResp
		switch scan_type {
		case "secret":
			req := http.Client().SecretScanAPI.StatusSecretScan(context.Background())
			req = req.ModelScanStatusReq(*toae_server_client.NewModelScanStatusReq(
				scan_id,
				[]string{},
			))
...
var scanListSubCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		var err error
		var res *toae_server_client.ModelScanListResp
		switch scan_type {
		case "secret":
			req := http.Client().SecretScanAPI.ListSecretScan(context.Background())
			req = req.ModelScanListReq(toae_server_client.ModelScanListReq{
				NodeIds: []toae_server_client.ModelNodeIdentifier{{NodeId: node_id, NodeType: node_type}},
				Window: toae_server_client.ModelFetchWindow{
					Offset: 0,
					Size:   20,
				},
			})
...
var scanSearchSubCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		scan_filters := toae_server_client.SearchSearchFilter{}
		if scan_filter != "" {
			orderFilter := toae_server_client.ReportersOrderFilter{}
			...
			scan_filters = toae_server_client.SearchSearchFilter{
				...
				Filters: toae_server_client.ReportersFieldsFilters{
					ContainsFilter: toae_server_client.ReportersContainsFilter{
						FilterIn: sfields,
					},
					MatchFilter: toae_server_client.ReportersMatchFilter{
		...
		node_filters := toae_server_client.SearchSearchFilter{}
		if node_filter != "" {
			orderFilter := toae_server_client.ReportersOrderFilter{}
		...
		var err error
		var res []toae_server_client.ModelScanInfo
		switch scan_type {
		case "secret":
			req := http.Client().SearchAPI.SearchSecretsScans(context.Background())
			req = req.SearchSearchScanReq(toae_server_client.SearchSearchScanReq{
...
var scanResultsSubCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		case "secret":
			...
			req = req.ModelScanResultsReq(toae_server_client.ModelScanResultsReq{
				ScanId: scan_id,
				Window: toae_server_client.ModelFetchWindow{
					Offset: 0,
					Size:   20,
		...
		case "vulnerability":
			...
			req = req.ModelScanResultsReq(toae_server_client.ModelScanResultsReq{
				ScanId: scan_id,
				FieldsFilter: toae_server_client.ReportersFieldsFilters{
					ContainsFilter: toae_server_client.ReportersContainsFilter{
...
var scanStopSubCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		stopReq := toae_server_client.ModelStopScanRequest{
			ScanIds: scan_ids,
		}
