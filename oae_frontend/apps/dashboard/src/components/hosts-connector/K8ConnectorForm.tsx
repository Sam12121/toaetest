...
const PLACEHOLDER_API_KEY = '---TOAE-API-KEY--';
...
const InformationForm = ({
  ...
  useMemo(() => {
    ...
    const installCommand = `helm install toae-agent toae/toae-agent \\
  --set managementConsoleUrl=${window.location.host ?? '---CONSOLE-IP---'} \\
  --set toaeKey=${dfApiKey} \\
  ...
  return (
    <div className="relative flex items-center">
      <pre className="h-fit text-p7 dark:text-text-text-and-icon">
        helm repo add toae
        https://toae-helm-charts.s3.amazonaws.com/threatmapper
      </pre>
      <div className="flex items-center ml-auto self-start">
        ...
        <IconButton
          ...
          onClick={() => {
            copy(
              'helm repo add toae https://toae-helm-charts.s3.amazonaws.com/threat',
            );
          }}
        />
      </div>
...
export const K8ConnectorForm = () => {
  const [instruction, setInstruction] =
    useState(`helm repo add toae https://toae-helm-charts.s3.amazonaws.com/threatmapper
helm repo update

helm install toae-agent toae/toae-agent \\
  --set managementConsoleUrl=${window.location.host ?? '---CONSOLE-IP---'} \\
  --set toaeKey=${PLACEHOLDER_API_KEY} \\
  --set global.imageTag=${''} \\
      ...
      <Stepper>
        ...
        >
          <div className="text-p7 dark:text-text-text-and-icon">
            ...
            <DFLink
              href={`https://toaesecurity.com/contact/threatmapper/docs/v2.1/sensors/kubernetes`}
              target="_blank"
              rel="noreferrer"
              className="mt-2"
            >
