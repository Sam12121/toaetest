...
usage() {
  ...
  cat <<EOF
	...
	OPTIONS:
        -h Show this message
        -r IP Address / domain of Toae management console (Mandatory)
        -o Port of Toae management console (Mandatory. Default is 443)
        -k Toae key for auth
        -n Hostname to use in toae agent (Optional)
...
IMAGE_REPOSITORY=${IMAGE_REPOSITORY:-docker.io/toaeio}
...
kill_agent() {
  agent_running=$(sudo podman ps --format '{{.Names}}' | grep "toae-agent")
  if [ "$agent_running" != "" ]; then
    sudo podman rm -f toae-agent
  fi
}
...
start_agent() {
  sudo podman run -dit \
    ...
    "$IMAGE_REPOSITORY"/toae_agent_ce:"${DF_IMG_TAG:-2.1.0}"
}
